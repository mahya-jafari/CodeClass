...
export function useIDE(canEdit = true) {
...
export default function IDEPanel({
  ...
  addFile: addFileProp,
  requestDeleteFile: requestDeleteFileProp,
  uploadFiles: uploadFilesProp,
  getLang: getLangProp,
}) {
  const file = ide?.file ?? fileProp;
  const setFile = ide?.setFile ?? setFileProp;
  const files = ide?.files ?? filesProp;
  const setFiles = ide?.setFiles ?? setFilesProp;
  const getLang = ide?.getLang ?? getLangProp;
  const addFile = ide?.addFile ?? addFileProp;
  const requestDeleteFile = ide?.requestDeleteFile ?? requestDeleteFileProp;
  const uploadFiles = ide?.uploadFiles ?? uploadFilesProp;

  return (
    <div className="flex-1 flex flex-col sm:flex-row overflow-hidden" dir="ltr">
      <div className="w-full sm:w-48 lg:w-56 bg-[#1e1e1e] text-gray-300 flex flex-col border-b sm:border-b-0 sm:border-r border-gray-700 flex-shrink-0 max-h-28 sm:max-h-none">
      ...
      <div className="flex-1 min-h-0">
        <Editor
          height="100%"
          theme="vs-dark"
          language={getLang?.(file) || "javascript"}
          value={files?.[file] || ""}
