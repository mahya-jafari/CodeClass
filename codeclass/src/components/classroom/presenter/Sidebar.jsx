    ...
    <aside className={`bg-white border-r flex flex-col flex-shrink-0 transition-all duration-300 overflow-hidden ${chatOpen ? "w-72 opacity-100" : "w-0 opacity-0 border-0"}`}>
      ...
      <div className="border-b flex-shrink-0">
        <div className="px-3 py-2 text-xs font-bold flex items-center gap-1.5">
          <FiUsers size={13} /> اعضای حاضر ({participants.length})
        </div>
        <div className="max-h-52 overflow-y-auto px-1.5 pb-1.5">
          {participants.map((p) => (
            <div key={p.id} className="relative flex items-center gap-1.5 px-1.5 py-1.5 rounded-lg hover:bg-gray-50">
              <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-medium flex-shrink-0">
                {p.name[0]}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-medium truncate">{p.name}</p>
                {!p.isSelf && (
                  <p className={`text-[9px] ${p.canEdit ? "text-green-600" : "text-gray-400"}`}>
              ...
              {/* mic */}
              <button
                onClick={() => setParticipants((ps) => ps.map((x) => x.id === p.id ? { ...x, mic: !x.mic } : x))}
