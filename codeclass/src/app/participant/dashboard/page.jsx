...
import {
  ...
  FiMessageSquare, FiSettings, FiClock
} from "react-icons/fi";
...
export default function ParticipantDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] flex" dir="rtl">
      <ParticipantSidebar activeMenu="dashboard" setActiveMenu={() => {}} menuItems={menuItems} isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 mb-6 sm:mb-8">
            {stats.map((s, i) => (
              <div key={i} className="bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl ${s.bg} flex items-center justify-center ${s.color}`}>{s.icon}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onClick={() => router.push("/participant/my-classes")} className="text-sm text-blue-600 hover:underline">مشاهده همه</button>
              </div>
              <div className="divide-y divide-gray-100">
                {myClasses.map((c) => (
                  <div key={c.id} onClick={() => router.push(`/participant/classroom/${c.id}`)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2">
                        <div className="flex justify-between text-xs text-gray-500 mb-1">
                          <span>پیشرفت</span><span>{c.progress}%</span>
                        </div>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                  ...
                  <button onClick={() => router.push("/participant/calendar")} className="text-xs text-blue-600 hover:underline">مشاهده همه</button>
                </div>
                <div className="p-4 space-y-3">
                  {upcoming.map((s) => (
                    <div key={s.id} onClick={() => router.push("/participant/classroom/1")}
                      className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
                      <div className="w-9 h-9 rounded-lg bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-600">{s.tag}</div>
                      <div className="flex-1 min-w-0">
