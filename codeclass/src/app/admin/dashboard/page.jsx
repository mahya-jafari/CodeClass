        ...
        <div className="p-4 sm:p-6 lg:p-8">
              ...
              <div key={i} className="bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className={`text-xs mt-1 flex items-center gap-1 ${s.change.startsWith("+") ? "text-green-600" : "text-gray-400"}`}>
                    <FiTrendingUp size={12} /> {s.change} نسبت به ماه قبل
                  </p>
                </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onClick={() => router.push("/admin/users")} className="text-sm text-indigo-600 hover:underline">
                  مشاهده همه
                </button>
              </div>
              <div className="divide-y divide-gray-100">
                {recentUsers.map((u) => (
                  <div key={u.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition">
                    ...
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${u.status === "فعال" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                      {u.status}
                    </span>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onClick={() => router.push("/admin/finance")} className="text-xs text-indigo-600 hover:underline">
                  همه
                </button>
              </div>
                  ...
                  pendingWithdrawals.map((w) => (
                    <div key={w.id} className="p-3 bg-gray-50 rounded-xl">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <p className="text-sm font-bold text-gray-800">{w.amount.toLocaleString("fa-IR")} تومان</p>
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mt-6">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={() => router.push("/admin/classes")} className="text-sm text-indigo-600 hover:underline">
                مشاهده همه
              </button>
            </div>
            <div className="divide-y divide-gray-100">
              {recentClasses.map((c) => (
                <div key={c.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">{c.teacher} · {c.students} دانشجو</p>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${c.status === "فعال" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}>
                    {c.status}
                  </span>
                </div>
              ))}
