            ...
            {isLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  ...
                  <tbody>
                    {filtered.map((a) => (
                      <tr key={a.id} className="block md:table-row border-t hover:bg-gray-50">
                        <td className="block md:table-cell p-4 lg:p-5">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="font-medium text-gray-800">{a.user}</div>
                          </div>
                        </td>
                        <td className="block md:table-cell px-4 pb-2 md:p-5 text-gray-700">{a.title}</td>
                        <td className="block md:table-cell px-4 pb-2 md:p-5 text-sm text-gray-500">{a.course}</td>
                        <td className="block md:table-cell px-4 pb-2 md:p-5 text-sm text-gray-500">{a.date}</td>
                        ...
                        <td className="block md:table-cell p-4 md:p-5">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${a.status === 'تحویل شده' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                            {a.status}
                          </span>
                        </td>
                      </tr>
