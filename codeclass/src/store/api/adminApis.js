...
export const adminApis = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // Presenter approvals
    getPendingPresenters: builder.query({
      query: () => "/admin/presenters/pending",
      providesTags: ["Presenters"],
    }),
    updatePresenterApproval: builder.mutation({
      query: ({ id, approvalStatus }) => ({
        url: `/admin/presenters/${id}/approval`,
        method: "PATCH",
        body: { approvalStatus },
      }),
      invalidatesTags: ["Presenters", "User"],
    }),
